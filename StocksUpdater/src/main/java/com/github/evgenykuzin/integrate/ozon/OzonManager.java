...
public class OzonManager implements Loggable {
    ...
    public void updateProductStocks(Collection<OzonProduct> ozonProducts) throws IOException, InterruptedException {
        var jsonBuilder = new JsonBuilder()
                .addNewArr("stocks");
        var mainQueue = new ArrayDeque<OzonProduct>(ozonProducts.size());
        mainQueue.addAll(ozonProducts);
        System.out.println("mainQueue = " + mainQueue.getFirst());
        while (!mainQueue.isEmpty()) {
            var queue = new ArrayDeque<OzonProduct>(20);
